...
- **The "no X, no Y, it just..." cadence.** A pair of negated phrases teeing up a breezy "it just [verb]s" payoff: "No rate limits, no per-minute bill, it just churns through audio while I do something else." Only two items, so it ducks the rule-of-three check, but the "no..., no..." anaphora plus the effortless "it just" closer is the same engineered marketing rhythm. The negations promise "none of the bad stuff" and "it just" hand-waves at how easy it all is. Cut to the one benefit that actually matters, in a plain clause, and lose the word "just": "I can leave hours of audio grinding through it and the only bill is the electricity."
- **Don't stack trailing appositives on a noun for texture.** One vivid detail is plenty ("a janky old laptop"). The tell is the second and third elaboration piled on with commas, and especially a "the kind of X you'd [verb]" flourish that editorializes the detail instead of stating it: "a janky old laptop with a GTX 1050 in it, three gigs of VRAM, the kind of GPU you'd find in a drawer and not bother selling." Keep the one concrete thing and stop the sentence: "a janky old laptop with a GTX 1050 in it." The reader doesn't need the spec sheet or the little story about the drawer.
- **Don't gloss a concrete statement with an abstracting re-label.** After saying the concrete thing, AI adds a sentence that restates it as a Big Concept: "deciding which moments are actually any good. That's reading and taste, the kind of judgment you can't write rules for." The second sentence only re-describes the first at a loftier altitude ("taste", "judgment you can't write rules for") and underlines a point the concrete already landed. Omit it entirely.
- **"the kind of X (that) you [verb]" is a recurring flourish - treat it as a yellow flag wherever it appears.** It showed up two ways in one draft: as appositive texture on a noun ("the kind of GPU you'd find in a drawer") and as a whole re-labeling sentence ("the kind of judgment you can't write rules for"). Both got cut. If you've written "the kind of X that you...", you're almost always editorializing a detail that should just be stated or dropped.
- **Don't end a section by restating its own moral.** The H2 already said "the same app gave me two opposite answers" and the paragraphs showed it happening. Then the draft spelled it out anyway: "Same app, same me, and "should I run this locally?" came back with two different answers because the shape of the work was different each time." Two tells stacked: the parallel-fragment drumroll ("Same app, same me,") that announces a profound point is coming, and the moral itself, which the reader already drew. End on the last concrete line ("nothing I can run at home comes anywhere close") and trust them with the conclusion. The "Same X, same Y," setup is a yellow flag on its own.

## More slop tells (caught while drafting, 2026-08)

- **No logline openers.** The worst offender from the perbrowser draft: "My VPN died on a Tuesday and told me it was fine." Three tells welded into one sentence. The object is given intent ("told me it was fine"), the sentence sets up and resolves its own little paradox (dead but reporting fine) before the reader has any stake in it, and it's built for quotability rather than typed by a person. A first line should put you somewhere doing something: "One morning the browser I run through Mullvad stopped loading anything." The mystery is allowed to be a mystery for another two sentences.
- **Don't date-stamp an anecdote for texture.** "on a Tuesday", "at 2am on a Thursday", "that March". If the day doesn't change the story, it's set dressing meant to make the anecdote feel lived-in, and it does the opposite. Say "one morning" or nothing at all.
- **Don't turn your own anecdote into a proper noun.** Having opened with a Tuesday, that draft went on to call it "the Tuesday incident" twice, as if the reader had been there. Naming your anecdote and then referring back to the name is a way of insisting it was significant. Refer back plainly ("that lost morning") or just restate the fact.
- **No "that's when I realized" epiphany closer.** From the same draft: "Fixing it meant rebuilding the whole thing by hand, and the launcher script alone carries three Chromium flags I can never remember. That's when I admitted this setup wanted to be a tool." The paragraph is written backwards from its own moral. A supporting detail gets picked to justify the turn (note "alone" doing the emphasis work), and then a retrospective-realization sentence tells the reader that This Was The Moment, which is the writing equivalent of a laugh track. Skip the annotation and do the next thing: "I rebuilt it by hand and then wrote a script so I would never have to do that again." Same ban on "that's when it clicked", "that was the moment I knew", "and that's how X was born".
- **No essay thesis sentence at the top of a section.** "The design that works runs entirely as an unprivileged user process, with nothing touching your system's network config." Nobody is in this sentence. "The design that works" is a definite-article abstraction that announces a Correct Answer from nowhere (same family: "The trick is", "The insight here is", "The approach that wins"), and it summarizes the paragraph the reader is about to read anyway, which is the topic-sentence habit from school essays. Then it closes with a tacked-on "with nothing touching..." phrase that exists for balance. Either cut the sentence and let the concrete paragraph lead, or put yourself in it: "Going in, I wanted the fewest moving parts I could get away with, and nothing that needed root."
- **Watch negation-as-benefit.** "no root, no kernel extensions, no changes to your network config" sells by listing absent bad things, which is marketing cadence, and it stacks into a rule-of-three on top. Name the one absence that actually matters to a reader and move on.
