...
What changes is the machine running that bytecode. The Monad client was written from scratch. It executes transactions in parallel and you don't even need to know it. It just watches what each transaction reads and re-runs the ones that conflict. The state is stored in MonadDb. Most Ethereum clients keep the Merkle Patricia trie inside a generic key-value database such as LevelDB or RocksDB, so one data structure ends up embedded in another in an inefficient way. MonadDb implements the Patricia trie directly, both on disk and in memory. I personally believe that if Ethereum had built the EVM in 2026 they would also make the same decision. The block time is 300ms and the finality is 600ms. And the best thing here is that you don't need to know what's happening behind the scenes.
...
I made a one-pager for this question a while ago: [whymonad.com](https://whymonad.com/). I got tired of saying "parallel EVM, 10k TPS, 300ms blocks". As far as I can tell, numbers by themselves don't make anyone switch chains. People switch when they try to build something and it doesn't work where they are. So the app shows three situations and plays them out on Ethereum, Solana, Arbitrum and Monad.
...
On Ethereum the txs queue and fees go up, because sequential execution and a generic state database cap the chain at about 15 transactions a second. On Solana execution is fast, but it has halted previously, and may not be as decentralized as Ethereum. One example is that Solana has also gone down under this kind of load before, 17 hours in 2021 from a bot flood. On Arbitrum things are cheap and you get a soft confirmation in about 2 seconds. But it all goes through one sequencer hence it is not wrong to say that it is pretty centralized, and if you want to actually leave to L1 that takes about 7 days.
